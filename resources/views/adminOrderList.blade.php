...
@section('content')
    <div class="container-fluid mt-4">
        <h4 class="py-1">Orders</h4>
        <div class="row">
            <div class="col-12">
                <div class="row">
                    <div class="col-12 table-responsive">
                        <form class="d-flex w-25" method="POST" action="{{route('order.search')}}">
                            @csrf
                        ...
                        <table class="table table-bordered align-middle overflow-auto mt-2">
                            <thead>
                                ...
                                <tr>
                                    <td>
                                        <form method="post" action="{{route('by.id')}}">
                                            @csrf
                                          
                                            @if(Session::get('id')=='asc')
                                            <select onchange="this.form.submit()" id="id" name="name" class="form-select" aria-label="Default select example">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($order->status == 'pending')
                                                <button onclick="location='{{ route('admin.status', ['id' => $order->id]) }}'"
                                                    class="btn btn-sm btn-warning">{{ $order->status }}</button>
                                            @else
