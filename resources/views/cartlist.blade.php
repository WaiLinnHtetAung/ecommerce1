            ...
            <div class="col-md-12 col-lg-8 col-12">
                @if (Cart::instance('shoppingcart')->count() > 0)
                    <div class="d-flex mt-3 justify-content-between align-items-center" style="margin-right:12px">
                        <h3 class="pb-1">My CartList ( {{ Cart::instance('wishlist')->count() }} )</h3>
                ...
                @if (Cart::instance('shoppingcart')->count() > 0)
                    @foreach (Cart::instance('shoppingcart')->content() as $item)
                        <div class="row border p-4 w-100 mt-3 ">
                            <div class="col-4">
                                {{-- {{dd($item->toArray())}} --}}
                                <img src="{{ $item->options['url'] }}" style="width:auto;height:100px" class="img-fluid"
                                    alt="...">
                            </div>
                            <div class="col-4 d-flex flex-column justify-content-center">
                                ...
                                <div class="btn-group pt-1" role="group" aria-label="Basic example">
                                    <button type="button" class="btn btn-outline-primary remove"
                                        id="{{ $item->rowId }}">-</button>
                            ...
                            <div class="col-4 d-flex position-relative">
                                <div class="position-absolute end-0 top-0">
                                    <button class="btn btn-danger removebtn" id="{{ $item->rowId }}">
                    ...
                    <div class="d-flex justify-content-end p-2">
                        <a href="{{ route('cart.removeall') }}" type="submit" class="btn btn-danger btn-sm mt-3"
    ...
    <script>
        ...
        $(document).ready(function(e) {
            $('.remove').click(function(e) {
                e.preventDefault();
                let number = parseInt(($('.num').html()))
                if (number > 0) {
                    let rowId = this.id;
                    let url = "{{ route('cart.decrease', ':id') }}";
